      ...
      <header className={`z-40 w-full sticky top-0 transition-all duration-300 ${
        ...
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4 transition-all duration-300 ${
          ...
          <div className="flex items-center gap-3.5 flex-shrink-0">
            {/* Streak Pill — opens StreakModal */}
            {userInfo?.userId && (
              <button
                onClick={() => setIsStreakModalOpen(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold select-none shadow-inner hover:bg-amber-500/20 hover:border-amber-500/50 transition-all duration-200 cursor-pointer focus:outline-none"
